    ...
    <div className="space-y-3 rounded-md border p-3">
      ...
      {props.allowAuto ? (
        <div className="flex flex-wrap gap-2">
          ...
          >
            {t("components.knowledge.documentPicker.auto")}
          </button>
          <button
            type="button"
            className={`rounded-md border px-3 py-1 text-sm ${!isAuto ? "bg-accent" : ""}`}
          ...
          >
            {t("components.knowledge.documentPicker.custom")}
          </button>
        </div>
      ) : null}
      ...
      {isAuto ? (
        <div className="rounded-md border border-dashed p-3 text-sm text-muted-foreground">
          {t("components.knowledge.documentPicker.autoHint")}
        </div>
      ) : (
        <>
          <Input
            ...
            placeholder={t("components.knowledge.documentPicker.searchPlaceholder")}
          />
          <div className="max-h-64 space-y-2 overflow-auto rounded-md border p-2">
            {documentsQuery.isLoading ? (
              <div className="text-sm text-muted-foreground">{t("components.knowledge.documentPicker.loading")}</div>
            ) : null}
            {visibleDocuments.length === 0 && !documentsQuery.isLoading ? (
              <div className="text-sm text-muted-foreground">{t("components.knowledge.documentPicker.empty")}</div>
            ) : null}
          ...
          <div className="text-xs text-muted-foreground">
            {t("components.knowledge.documentPicker.selectedCount", { count: selectedIds.length })}
          </div>
        </>
      )}
    </div>
