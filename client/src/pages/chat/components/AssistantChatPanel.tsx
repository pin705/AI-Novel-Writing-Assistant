  ...
  return message.content
    .map((part) => {
      ...
      if (part.type === "tool-call") {
        return t("[工具:{{toolName}}]", { toolName: part.toolName });
      }
      if (part.type === "data") {
        try {
      ...
      if (part.type === "image") {
        return t("[图片:{{value}}]", { value: part.filename ?? t("未命名") });
      }
      if (part.type === "file") {
        return t("[文件:{{value}}]", { value: part.filename ?? t("未命名") });
      }
      return "";
    })
    .filter(Boolean)
        ...
        try {
          ...
          const persistedMessages = [
            ...toChatMessages(options.messages),
            {
              ...
              content: finalAssistantText || t("（空响应）"),
          ...
          streamError = error instanceof Error ? error.message : t("消息发送失败。");
      ...
      <ThreadPrimitive.Root className="space-y-4">
        <ThreadPrimitive.Viewport className="max-h-[52vh] space-y-4 overflow-auto rounded-2xl bg-gradient-to-b from-slate-50 to-slate-100/70 p-4 ring-1 ring-slate-200">
          <ThreadPrimitive.Empty>
            <div className="mx-auto mt-8 max-w-[680px] px-2 text-center">
              <h3 className="text-4xl font-semibold tracking-tight text-slate-900">{t("你好！")}</h3>
              <p className="mt-2 text-2xl text-slate-500">{t("今天想一起完善哪段剧情？")}</p>
              <div className="mt-8 grid gap-3 md:grid-cols-2">
                ...
                >
                  ...
                  >
                    <div className="text-sm font-medium text-slate-900">{t("世界观一致性检查")}</div>
                    <div className="mt-1 text-xs text-slate-500">{t("快速识别硬冲突并给出修复方向")}</div>
                  </button>
                </ThreadPrimitive.Suggestion>
                <ThreadPrimitive.Suggestion
                ...
                >
                  <button
                    ...
                    className="rounded-2xl border border-slate-200 bg-white p-4 text-left transition hover:border-slate-300 hover:bg-slate-50"
                  >
                    <div className="text-sm font-medium text-slate-900">{t("章节重写草案")}</div>
                    <div className="mt-1 text-xs text-slate-500">{t("聚焦结尾张力与角色一致性")}</div>
                  </button>
                </ThreadPrimitive.Suggestion>
              </div>
            </div>
        ...
        <ComposerPrimitive.Root className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm">
          <ComposerPrimitive.Input
            ...
            placeholder={t("输入消息并回车发送，Shift+Enter 换行。")}
            submitMode="enter"
          />
