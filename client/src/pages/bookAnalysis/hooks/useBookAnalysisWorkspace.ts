...
export function useBookAnalysisWorkspace(): BookAnalysisWorkspace {
  ...
  const publishMutation = useMutation({
    ...
    onSuccess: async (response, payload) => {
      ...
      setPublishFeedback(
        t("bookAnalysis.detail.publish.feedbackSuccess", {
          documentId: published.knowledgeDocumentId,
          version: published.knowledgeDocumentVersionNumber,
          count: published.bindingCount,
        }),
      );
      await queryClient.invalidateQueries({ queryKey: queryKeys.knowledge.documents("book-analysis-source") });
    ...
    onError: (error) => {
      const message = error instanceof Error
        ? error.message
        : t("bookAnalysis.detail.publish.feedbackFailedFallback");
  ...
  const createStyleProfileMutation = useMutation({
    ...
    onMutate: () => {
      setStyleProfileFeedback(t("bookAnalysis.detail.styleProfile.feedbackStart"));
    },
    onSuccess: async (response) => {
      ...
      toast.success(t("bookAnalysis.detail.styleProfile.successToast"));
    ...
    onError: (error) => {
      const message = error instanceof Error
        ? error.message
        : t("bookAnalysis.detail.styleProfile.feedbackFailedFallback");
  ...
  const createStyleProfileFromAnalysis = async () => {
    ...
    await createStyleProfileMutation.mutateAsync({
      ...
      name: t("bookAnalysis.detail.styleProfile.namePattern", { title: selectedAnalysis.title }),
    });
  };
